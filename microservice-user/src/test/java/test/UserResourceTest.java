...
public class UserResourceTest {
    ...
    public static void setup() throws Exception {
        ...
        baseUrl = "https://" + System.getProperty("liberty.test.hostname") + ":" + System.getProperty("liberty.test.ssl.port");
    }
    ...
    public void testCreateUser() throws Exception {
        // Test1: Add user.
    	String loginAuthHeader = "Bearer " + new JWTVerifier().createJWT("unauthenticated", new HashSet<String>(Arrays.asList("login")));
        User user1 = new User(null, "Isaac", "Newton", "inewton", "@inewton", "inewtonWishListLink", "mypassword");
        ...
        // Validate user.
        BasicDBObject dbUser = (BasicDBObject) database.getCollection("users").findOne(new ObjectId(dbId));
        ...
        User user2 = new User(null, "Ivan", "Newton", "inewton", "@ivannewton", "ivannewtonWishListLink", "myPassword");
        response = processRequest(url, "POST", user2.getJson(), loginAuthHeader);
        assertEquals("HTTP response code should have been " + Status.BAD_REQUEST.getStatusCode() + ".",
    ...
    public void testCreateUserInvalidJwtGroup() throws Exception {
    	...
    	String loginAuthHeader = "Bearer " + new JWTVerifier().createJWT("unauthenticated", new HashSet<String>(Arrays.asList("users")));
        User user1 = new User(null, "Isaac", "Newton", "inewton", "@inewton", "inewtonWishListLink", "mypassword");
        String url = baseUrl + "/users/";
        
        // This request should fail since the JWT is in the wrong group.
        Response response = processRequest(url, "POST", user1.getJson(), loginAuthHeader);
        assertEquals("HTTP response code should have been " + Status.UNAUTHORIZED.getStatusCode() + ".",
    ...
    public void testUpdateUser() throws Exception {
    	...
    	String loginAuthHeader = "Bearer " + new JWTVerifier().createJWT("unauthenticated", new HashSet<String>(Arrays.asList("login")));
    	String url = baseUrl + "/users/";
        User user = new User(null, "James", "Maxwell", "jMaxwell", "@jMaxwell", "jMaxwellWishListLink", "myPassword");
        ...
        User updatedUser1 = new User(dbId, "James", "Maxwell", "jamesMaxwell", "@jamesMaxwell", "jamesMaxwellWishListLink", "newPassword");
        ...
        User updatedUser3 = new User(null, "Non", "Existent", "NonExistentUserName", "@ne", "neWishListLink", "noPassword");
    ...
    public void testDeleteUser() throws Exception {
    	...
    	String loginAuthHeader = "Bearer " + new JWTVerifier().createJWT("unauthenticated", new HashSet<String>(Arrays.asList("login")));
    	String url = baseUrl + "/users/";
        User user = new User(null, "Albert", "Einstein", "aEinstein", "@aEinstein", "aEinsteinWishListLink", "myPassword");
        Response response = processRequest(url, "POST", user.getJson(), loginAuthHeader);
        assertEquals("HTTP response code should have been " + Status.OK.getStatusCode() + ".",
                Status.OK.getStatusCode(), response.getStatus());
        String authHeader = response.getHeaderString("Authorization");
    ...
    public void testGetSingleUser() throws Exception {
        ...
        User user = new User(null, "Richard", "Feynman", "rFeynman", "@rFeynman", "rFeynmanWishListLink", "myPassword");
    ...
    public void testGetAllUser() throws Exception {
        ...
        User gGalilei = new User(null, "Galileo", "Galilei", "gGalilei", "@gGalilei", "gGalileiWishListLink", "myPassword");
        ...
        User mCurie = new User(null, "Marie", "Curie", "mCurie", "@mCurie", "mCurieWishListLink", "myPassword");
        ...
        BasicDBObject dbUser = (BasicDBObject) database.getCollection("users").findOne(new ObjectId(dbId1));
        ...
        assertTrue("Response payload is null. Payload containing list of users is expected.", payload != null);
        ...
        for (int i = 0; i < users.size(); i++) {
        	...
        	assertFalse("Response contained a password for user " + userName, responseUserObject.containsKey("password"));
            if (userName.equals("gGalilei")) {
                assertTrue("User gGalilei does not contain expected data.", gGalilei.isEqual(responseUserObject));
            } else if (userName.equals("mCurie")) {
                assertTrue("User mCurie does not contain expected data.", mCurie.isEqual(responseUserObject));
            } else {
