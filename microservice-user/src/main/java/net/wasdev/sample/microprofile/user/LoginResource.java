...
/**
 * This resource is responsible for logging in a user.  Upon a successful
 * login, the ID of the user will be returned.  Both regular login with
 * username and password, as well as Twitter login, are supported.
 */
@Path("/logins")
@RequestScoped
public class LoginResource {
    ...
    /**
     * The JWT of the current caller.  Since this is a request scoped resource,
     * the JWT will be injected for each JAX-RS request.  The injection is
     * performed by the mpJwt-1.0 feature.
     */
    @Inject
    private JsonWebToken jwtPrincipal;

    /**
     * A location to store twitter oauth_secret used during the login process.
     * Twitter requires a secret to be obtained in phase one, and then used in
     * phase two to sign the login request parameters.
     *
     * Note that this map does not work when this microservice is scaled out. In
     * the event two of these services are running, a separate caching service
     * must be used.
     */
    private static final Map<String, RequestToken> twitterOauthLoginMap = new HashMap<String, RequestToken>();
    ...
    /**
     * The twitter consumer key for the twitter app representing this service.
     * The consumer key is used to log in the user via Twitter.
     * This value is injected by MP Config.  It is specified in the project POM, 
     * and copied into the Liberty server bootstrap.properties during the build.
    ...
    /**
     * The twitter consumer secret for the twitter app representing this service.
     * The consumer secret is used to log in the user via Twitter.
     * This value is injected by MP Config.  It is specified in the project POM, 
     * and copied into the Liberty server bootstrap.properties during the build.
     * bootstrap.properties is a pre-defined config source for MP config.
     */
    ...
    /**
     * The host serving the twitter sign-in callback URL.  Twitter requires a
     * callback URL to be used after Twitter receives the username and password
     * from the user.  The caller is then redirected to the callback, where our
     * application completes the sign-in process.
     * 
     * This value is injected by MP Config.  It is specified in the project POM, 
     * and copied into the Liberty server bootstrap.properties during the build.
     * bootstrap.properties is a pre-defined config source for MP config.
    ...
    /**
     * The port serving the twitter sign-in callback URL.
     * This value is injected by MP Config.  It is specified in the project POM, 
     * and copied into the Liberty server bootstrap.properties during the build.
     * bootstrap.properties is a pre-defined config source for MP config.
     */
    ...
    /**
     * This endpoint will attempt to log-in a user using the password specified
     * in the user database.  The caller must have a valid JWT obtained from the
     * auth service.
    ...
    public Response loginUser(JsonObject payload) {
    	...
    	try {
    		...
    		return Response.status(Status.UNAUTHORIZED).type(MediaType.TEXT_PLAIN).entity(jwte.getMessage())
    				.build();
    	}
        ...
        if (buildJwt) {
            try {
				okResponse.header(HttpHeaders.AUTHORIZATION, "Bearer " + buildJWT(userName))
    ...
    /**
     * This endpoint starts the Twitter login process.  The Twitter login is a
     * two-step process.  During the first step, the consumer key and secret used
     * by this application is provided to Twitter, and provides an oauth token and 
     * secret which is used in the second step.  The token is returned to the caller, 
     * the secret is stored for use during the second step.
     * @return The oauth token that the user must provide to Twitter during the second
     *         step.  The caller should provide the oauth token to Twitter during the
     *         login process, after which Twitter will redirect the user back to
    ...
    /**
     * This endpoint processes the second step of a Twitter login.  At the
     * conclusion of the first step, the user was redirected back here with
     * an oauth verifier.  The oauth verifier is combined with the oauth token
     * provided in the first step, and the oauth secret provided in the first
    ...
    public Response verifyTwitterUser(JsonObject payload) {
        ...
        try {
            ...
            if (dbUser == null) {
                ...
                responseBody = builder.add(LOGIN_RESPONSE_ID_KEY, dbEntry.getString(User.DB_ID))
                        .add(LOGIN_RESPONSE_TWITTER_KEY, true).build();
                buildJwt = true;
            } else {
    ...
    /**
     * Build the URL that Twitter will redirect the user to, after the first phase of the
     * login process.  The redirect is to the front end, so we build the URL here using the
     * hostname and port that the front end is using.
    ...
    private final Configuration buildTwitterConfiguration() {
        if ((twitterConsumerKey == null) || (twitterConsumerSecret == null) || (twitterConsumerKey.equals("CHANGE_ME"))
                || (twitterConsumerSecret.equals("CHANGE_ME"))) {
            return null;
        }

        return new ConfigurationBuilder().setOAuthConsumerKey(twitterConsumerKey)
                .setOAuthConsumerSecret(twitterConsumerSecret).build();
    }
    
    /**
     * Build a JWT that will be used by an authenticated user.  The JWT wil be in
     * the 'users' group and should contain the username as defined by MP JWT.
