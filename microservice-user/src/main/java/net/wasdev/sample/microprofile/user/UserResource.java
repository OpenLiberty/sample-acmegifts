...
public class UserResource {
    ...
    public Response addUser(JsonObject payload) {
        ...
        if ((userPassword == null) || (userPassword.trim().length() == 0)) {
            return Response.status(Status.BAD_REQUEST).entity("The request must contain a password").build();
        }
        ...
        JsonObjectBuilder builder = createJsonBuilder(payload)
        ...
        try {
        	jwtTokenString = JwtBuilder.create("jwtUserBuilder")
        ...
        if (dbCollection.findOne(new BasicDBObject(User.JSON_KEY_USER_NAME, user.getUserName())) != null) {
    ...
    public Response updateUser(@PathParam("id") String id, JsonObject payload) {
        ...
        if (payload.containsKey("password")) {
            try {
                ...
                JsonObject newJson = createJsonBuilder(payload)
                        .add(User.JSON_KEY_USER_PASSWORD_HASH, pwUtil.getHashedPassword())
                        .add(User.JSON_KEY_USER_PASSWORD_SALT, pwUtil.getSalt()).build();
     ...
     * @return The user's profile, as a JSON object. Private fields such as
     *         password and salt are not returned.
     */
    @GET
    @Path("/{id}")
