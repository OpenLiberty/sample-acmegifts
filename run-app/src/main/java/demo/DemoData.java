...
public class DemoData {
    ...
    private static void parseOccasions() throws IOException {
        final URL occasions = Thread.currentThread().getContextClassLoader().getResource(BOOT_FILE_OCCASIONS);
        ...
        for (JsonValue occasionJsonValue : jsonOccasions) {
            try {
                ...
                int fudge = OCCASION_DATE_FUDGE_LOWER_BOUND + random.nextInt(OCCASION_DATE_FUDGE_RANDOM_UPPER_BOUND);
     ...
     * payload.
     */
    private static Response makeConnection(String method, String urlString, String payload, String jwt)
    ...
    /**
     * Calculates a date (current date + fudge) and replaces the JSON entry of
     * 'year-month-day' with the calculated date. If the JSON input does not
     * contain the 'year-month-day' entry, the unmodified input JSON is
     * returned.
     *
     * @param jsonString The JSON string to operate on.
     * @param fudge A random number between 90 and 200.
     *
     * @return The updated JSON string input or the unmodified JSON input if it
     *         does not contain the expected 'year-month-day'
     */
