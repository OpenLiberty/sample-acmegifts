...
public class NotificationRetryBean {
    ...
    /**
     * Retry twice in the event of failure, after which the fallback handler
     * will be driven to write the message to a fallback log.
     */
    @Retry(maxRetries = 2)
    @Fallback(NotificationFallbackHandler.class)
    public String tweet(String message, Twitter twitter, Logger fbLogger, String handle, Logger logger)
            throws Exception {
        try {
    ...
    /**
     * Returns a list of strings that are less than or equal to 140 chars in
     * length. Partial/continuing strings are suffixed/prefixed with "... "
     * respectively.
     *
