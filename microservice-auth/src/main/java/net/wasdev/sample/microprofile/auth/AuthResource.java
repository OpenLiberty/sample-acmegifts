...
/**
 * The auth resource is responsible for providing a JWT to the caller, which can be
 * used to login with the user service, or create a user with the user service.
 * The JWT is a member of the "login" group and therefore can only access resources
 * which this group is permitted to.
 * 
 * In the future, the auth resource should be modified to contain all JWT creation.
 * Currently, the user service will issue a new JWT once the user has logged in,
 * and the occasion service will issue its own JWT when an occasion is ready to run.
 */
@Path("/auth")
@RequestScoped
public class AuthResource {
	...
	/**
	 * Make a JWT that can be used only to login or create a user.  The JWT will
	 * expire in a short amount of time, and the group should only allow a login.
	 */
	@GET
	public Response generateJwtLogin() {
        ...
        try {
        	jwtTokenString = JwtBuilder.create("jwtAuthLoginBuilder")
