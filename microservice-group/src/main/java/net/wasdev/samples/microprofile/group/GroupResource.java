...
public class GroupResource {
    ...
    public Response getGroupInfo(@PathParam("id") String id) {
        ...
        if (!ObjectId.isValid(id)) {
            return Response.status(Status.BAD_REQUEST).type(MediaType.TEXT_PLAIN).entity("The group was not found")
                    .build();
        }
        ...
        if (userId != null) {
            if (!ObjectId.isValid(userId)) {
                return Response.status(Status.BAD_REQUEST).type(MediaType.TEXT_PLAIN)
                        .entity("The user id provided is not valid.").build();
            }
    ...
    public Response deleteGroup(@PathParam("id") String id) {
        ...
        // throw a 500
        if (!ObjectId.isValid(id)) {
            return Response.status(Status.BAD_REQUEST).type(MediaType.TEXT_PLAIN).entity("The group ID is not valid.")
                    .build();
        }
