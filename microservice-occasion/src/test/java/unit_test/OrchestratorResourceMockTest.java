...
public class OrchestratorResourceMockTest {
    ...
    public void testEventNotification_OneContribution() throws IOException {
        ...
        String groupResponseJson = buildGroupResponseObject(groupName, new String[] { userId },
                new String[] { occasion.getId().toString() });
        Response groupResponse = Response.ok().entity(groupResponseJson).build();

        String userResponseJson = buildUserResponseObject(userFirstName, userLastName, userName, userTwitterHandle,
                userWishListLink, new String[] { groupId });
        ...
        doReturn(groupResponse).when(mock_orchestrator).makeConnection("GET", GROUP_SERVICE_URL, null, jwtString);
        doReturn(userResponse).when(mock_orchestrator).makeConnection("GET", USER_SERVICE_URL, null, jwtString);
        doReturn(emailContent).when(mock_orchestrator).createEventNotificationMessage(userFirstName, userLastName,
                userWishListLink, groupName, name, NumberFormat.getCurrencyInstance(Locale.US).format(contribution));
        doReturn(Response.ok().build()).when(mock_orchestrator).makeConnection(eq("POST"), eq(NOTIFICATION_SERVICE_URL),
                any(String.class), eq(jwtString));
        ...
        verify(mock_orchestrator, times(1)).createEventNotificationMessage(userFirstName, userLastName,
                userWishListLink, groupName, name, NumberFormat.getCurrencyInstance(Locale.US).format(contribution));
        verify(mock_orchestrator, times(1)).makeConnection("POST", NOTIFICATION_SERVICE_URL, notificationRequestJson,
                jwtString);
    ...
    public void testEventNotification_TwoContributions() throws IOException {
        ...
        String name = "Jane's Birthday";
        Occasion occasion = generateOccasion(name, groupId, user1Id,
        ...
        // Setup mocked JSON payloads
        String groupResponseJson = buildGroupResponseObject(groupName, new String[] { user1Id, user2Id },
                new String[] { occasion.getId().toString() });
        Response groupResponse = Response.ok().entity(groupResponseJson).build();

        String user1ResponseJson = buildUserResponseObject(user1FirstName, user1LastName, user1Name, user1Email,
                user1WishListLink, new String[] { groupId });
        ...
        Orchestrator mock_orchestrator = Mockito.spy(orchestrator);
        doReturn(groupResponse).when(mock_orchestrator).makeConnection("GET", GROUP_SERVICE_URL, null, jwtString);
        doReturn(user1Response).when(mock_orchestrator).makeConnection("GET", USER_SERVICE_URL, null, jwtString);
        doReturn(emailContent).when(mock_orchestrator).createEventNotificationMessage(user1FirstName, user1LastName,
                user1WishListLink, groupName, name,
        ...
        verify(mock_orchestrator, times(1)).makeConnection("GET", GROUP_SERVICE_URL, null, jwtString);
        verify(mock_orchestrator, times(1)).makeConnection("GET", USER_SERVICE_URL, null, jwtString);
        verify(mock_orchestrator, times(1)).createEventNotificationMessage(user1FirstName, user1LastName,
                user1WishListLink, groupName, name,
                NumberFormat.getCurrencyInstance(Locale.US).format(totalContributionAmount));
        verify(mock_orchestrator, times(1)).makeConnection("POST", NOTIFICATION_SERVICE_URL, notificationRequestJson,
                jwtString);
    ...
    private String buildUserResponseObject(String firstName, String lastName, String userName, String twitterHandle,
            String wishListLink, String[] groups) {
     ...
     */
    private Occasion generateOccasion(String name, String groupId, String recipientId, double[] contributionAmounts) {
        ...
        return new Occasion(id, null, /* String date */
