...
public class NotificationFallbackHandler implements FallbackHandler<OccasionResponse> {
    ...
    public OccasionResponse handle(ExecutionContext context) {
		...
		try {
			notificationResponse = orchestrator.makeConnection("POST", notification11ServiceUrl, payload, jwtTokenString);
		} catch (IOException e) {
			e.printStackTrace();
		}
		OccasionResponse occasionResponse = new OccasionResponse(notificationResponse, OccasionResponse.NOTIFICATION_TYPE_TWEET, null);
