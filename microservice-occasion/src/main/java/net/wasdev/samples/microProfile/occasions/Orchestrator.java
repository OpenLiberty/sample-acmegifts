...
public class Orchestrator {
    ...
    /**
     * Local reference to the occasion resource that we can use to modify the
     * occasion database.
     */
    private OccasionResource occasionResource = null;

    @Inject
    private NotificationRetryBean notificationRetryBean;
    ...
    public void scheduleOccasion(Occasion occasion) throws ParseException {
        ...
        Future<?> scheduledOccasion = executor.schedule(notify, Math.max(delay, 0), TimeUnit.MILLISECONDS);
    ...
    /**
     * Retrieve information from each user and create an email list to send to
    ...
    private OccasionResponse notify(Occasion occasion) {
        ...
        try {
            groupResponse = makeConnection("GET", GROUP_SERVICE_URL + "/" + occasion.getGroupId(), null,
                    jwtTokenString);
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        try {
            userResponse = makeConnection("GET", USER_SERVICE_URL + "/" + occasion.getRecipientId(), null,
                    jwtTokenString);
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        String message = createEventNotificationMessage(firstName, lastName, wishList, groupName, occasion.getName(),
                NumberFormat.getCurrencyInstance(Locale.US).format(totalAmount));

        try {
            occasionResponse = notificationRetryBean.makeNotificationConnection(message, this, jwtTokenString,
                    NOTIFICATION_1_1_SERVICE_URL, twitterHandle, NOTIFICATION_SERVICE_URL);
        } catch (Throwable t) {
            t.printStackTrace();
     ...
     */
    public Response makeConnection(String method, String urlString, String payload, String jwtTokenString)
            throws IOException {
        ...
        if (payload != null) {
            ...
            response = invoBuild.header(HttpHeaders.AUTHORIZATION, "Bearer " + jwtTokenString).build(method, data).invoke();

        } else {
            // No JSON payload to send
            Invocation.Builder invoBuild = target.request();
            response = invoBuild.header(HttpHeaders.AUTHORIZATION, "Bearer " + jwtTokenString).build(method).invoke();
        }

        return response;
    }

    public String createEventNotificationMessage(String recipientFirstName, String recipientLastName, String wishList,
            String groupName, String occasionName, String totalAmount) {
        return "Congratulations " + recipientFirstName + " " + recipientLastName + "! " + totalAmount
                + " has been contributed by " + groupName + " for " + occasionName
                + ".  Please select a gift from your wish list at " + wishList + ".";
    }
    ...
    public void setOccasionResource(OccasionResource occasionResource) {
        ...
        NOTIFICATION_SERVICE_URL = "http://" + notificationServiceHostname + ":" + notificationServicePort + "/notifications";
        NOTIFICATION_1_1_SERVICE_URL = "http://" + notification_1_1ServiceHostname + ":" + notification_1_1ServicePort + "/notifications";
