...
public class OccasionResource
{
    ...
    public Response createOccasion(JsonObject json)
    {
        ...
        else
        {
            ...
            if (null != id && !id.toString().isEmpty())
            {
                ...
                response = Response.status(400).entity("Create failed. Payload must not contain an ID. Recieved ID: \"" + id + "\"").build();
            }
            else
            {
                ...
                String jsonResp = new Occasion(occasionId, null, null, null, null, null, null, null).toString();
    ...
    public Response runOccasion(JsonObject json) {
        ...
        try {
            ...
            logger.exiting(clazz, method, Status.UNAUTHORIZED);
            return Response.status(Status.UNAUTHORIZED).type(MediaType.TEXT_PLAIN).entity(jwte.getMessage()).build();
        }
        ...
        if (null == json || json.toString().isEmpty()) {
            ...
            if (null == id || id.toString().isEmpty()) {
                ...
                response = Response.status(400)
                    .entity("Run failed. Payload must contain an ID.").build();
            } else {
                // Run the occasion
    ...
    Response buildPostRunResponse(OccasionResponse occasionResponse) {
        ...
        if (requestResponse.equals(OccasionResponse.NOTIFICATION_TYPE_LOG) || requestResponse.equals(OccasionResponse.NOTIFICATION_TYPE_TWEET)) {
    ...
    public Response retrieveOccasion(@PathParam("id") String id)
    {
        ...
        else
        {
            ...
            DBObject occasion = getCollection().findOne(new BasicDBObject(Occasion.OCCASION_ID_KEY, new ObjectId(id)));
    ...
    public Response retrieveGroupsForOccasion(@QueryParam("groupId") String groupId)
    {
        ...
        else
        {
            ...
            List<DBObject> occasionList = getCollection().find(new BasicDBObject(Occasion.OCCASION_GROUP_ID_KEY, groupId)).toArray();
    ...
    public Response updateOccasion(@PathParam("id") String id, JsonObject json)
    {
        ...
        {
            // perform the update using $set to prevent overwriting data in the event of an incomplete payload
            ...
            DBObject updatedObject = getCollection().findAndModify( query, null, null, false, updateObject, true, false );
            ...
            try {
                ...
                e.printStackTrace();
                response = Response.status(400).entity("Invalid date given. Format must be YYYY-MM-DD").build();
            }
        }
    ...
    public boolean deleteOccasion(ObjectId idObject) {
        WriteResult result = getCollection().remove(new BasicDBObject(Occasion.OCCASION_ID_KEY, idObject));
